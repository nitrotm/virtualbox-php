 ...
 */
static const char * apicModeConverter(PRUint32 value) {
    switch (value) {
 ...
 * Enumeration converter
 */
static const char * usbControllerTypeConverter(PRUint32 value) {
    switch (value) {
...
static void exportVirtualBoxBIOSSettings(IBIOSSettings *biosSettings, xmlTextWriterPtr writer) {
    ...
    // apicmode
    ADDXMLENUM(biosSettings->GetAPICMode, "apicmode", apicModeConverter);
}
...
static void exportVirtualBoxNetworkAdapter(INetworkAdapter *networkAdapter, PRUint32 index, xmlTextWriterPtr writer) {
    ...
    if (attachmentType != NetworkAttachmentType_Null) {
        // enabled
...
static void exportVirtualBoxAudioAdapter(IAudioAdapter *audioAdapter, xmlTextWriterPtr writer) {
    ...
    if (audioDriverType != AudioDriverType_Null) {
        // audio controller
 ...
 */
static void exportVirtualBoxUSBController(IUSBController *usbController, PRUint32 index, xmlTextWriterPtr writer) {
    // find info
    PRUint32 type = USBControllerType_Null;

    usbController->GetType(&type);

    switch (type) {
    case USBControllerType_OHCI:
        WRITEXMLBOOL("usb", PR_TRUE);
        ADDXMLSTRING(usbController->GetName, "usbname");
        ADDXMLINT16U(usbController->GetUSBStandard, "usbversion");
        break;

    case USBControllerType_EHCI:
        WRITEXMLBOOL("usbehci", PR_TRUE);
        ADDXMLSTRING(usbController->GetName, "usbehciname");
        ADDXMLINT16U(usbController->GetUSBStandard, "usbehciversion");
        break;

    case USBControllerType_XHCI:
        WRITEXMLBOOL("usbxhci", PR_TRUE);
        ADDXMLSTRING(usbController->GetName, "usbxhciname");
        ADDXMLINT16U(usbController->GetUSBStandard, "usbxhciversion");
        break;
    }
}
...
/**
 * Export VirtualBox usb filter
 */
static void exportVirtualBoxUSBFilter(IUSBDeviceFilter *usbFilter, PRUint32 index, xmlTextWriterPtr writer) {
    char name[256];
    nsresult rc;

    // active
    sprintf(name, "usbfilter%d", index);
    ADDXMLBOOL(usbFilter->GetActive, name);

    // name
    sprintf(name, "usbfiltername%d", index);
    ADDXMLSTRING(usbFilter->GetName, name);

    // vendorid
    sprintf(name, "usbfiltervendorid%d", index);
    ADDXMLSTRING(usbFilter->GetVendorId, name);

    // productid
    sprintf(name, "usbfilterproductid%d", index);
    ADDXMLSTRING(usbFilter->GetProductId, name);

    // manufacturer
    sprintf(name, "usbfiltermanufacturer%d", index);
    ADDXMLSTRING(usbFilter->GetManufacturer, name);

    // product
    sprintf(name, "usbfilterproduct%d", index);
    ADDXMLSTRING(usbFilter->GetProduct, name);

    // serial number
    sprintf(name, "usbfilterserialnumber%d", index);
    ADDXMLSTRING(usbFilter->GetSerialNumber, name);
}


/**
 * Export VirtualBox usb filters
 */
static void exportVirtualBoxUSBFilters(IUSBDeviceFilters *usbFilters, xmlTextWriterPtr writer) {
    IUSBDeviceFilter **filters = nsnull;
    PRUint32 filtersCount = 0;
    nsresult rc;

    usbFilters->GetDeviceFilters(&filtersCount, &filters);

    // total
    WRITEXMLINT32("usbfilters", filtersCount);

    // filters
    if (NS_SUCCEEDED(rc)) {
        for (PRUint32 i = 0; i < filtersCount; i++) {
            exportVirtualBoxUSBFilter(filters[i], i + 1, writer);
        }
    }

    NS_FREE_XPCOM_ISUPPORTS_POINTER_ARRAY(filtersCount, filters);
}
    ...
    if (NS_SUCCEEDED(rc)) {
        xmlTextWriterStartElement(writer, TOXMLCHAR("machine"));
            ...
            // long-mode
            {
                ...
                rc = machine->GetCPUProperty(CPUPropertyType_LongMode, &value);
                if (NS_SUCCEEDED(rc)) {
                    WRITEXMLBOOL("longmode", value);
                }
            }
            ...
            // triple-fault-reset
            {
                ...
                rc = machine->GetCPUProperty(CPUPropertyType_TripleFaultReset, &value);
                if (NS_SUCCEEDED(rc)) {
                    WRITEXMLBOOL("triplefaultreset", value);
                }
            }
            ...
            // apic
            {
                ...
                rc = machine->GetCPUProperty(CPUPropertyType_APIC, &value);
                if (NS_SUCCEEDED(rc)) {
                    WRITEXMLBOOL("apic", value);
                }
            }
            ...
            // x2apic
            {
                ...
                rc = machine->GetCPUProperty(CPUPropertyType_X2APIC, &value);
                if (NS_SUCCEEDED(rc)) {
                    WRITEXMLBOOL("x2apic", value);
                }
            }
            ...
            // hwvirtex
            {
                ...
                rc = machine->GetHWVirtExProperty(HWVirtExPropertyType_Enabled, &value);
                if (NS_SUCCEEDED(rc)) {
            ...
            // usb controllers
            {
                IUSBController **usbControllers = nsnull;
                PRUint32 usbControllersCount;

                rc = machine->GetUSBControllers(&usbControllersCount, &usbControllers);
                if (NS_SUCCEEDED(rc)) {
                    for (PRUint32 i = 0; i < usbControllersCount; i++) {
                        exportVirtualBoxUSBController(usbControllers[i], i + 1, writer);
                    }
                }

                NS_FREE_XPCOM_ISUPPORTS_POINTER_ARRAY(usbControllersCount, usbControllers);
            }

            // usb filters
            {
                nsCOMPtr<IUSBDeviceFilters> value;

                rc = machine->GetUSBDeviceFilters(getter_AddRefs(value));
                if (NS_SUCCEEDED(rc)) {
                    exportVirtualBoxUSBFilters(value, writer);
                }
            }
