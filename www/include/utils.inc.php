...
function readUSBInfo() {
    $usb = array();
    foreach (simpleExec('/usr/bin/lsusb', array()) as $line) {
        if (preg_match("/^Bus\s+(\d+)\s+Device\s+(\d+):\s+ID\s+([0-9a-f]+):([0-9a-f]+)\s+(.+)$/", $line, $matches)) {
            $usb[] = array(
                'bus' => intval($matches[1]),
                'device' => intval($matches[2]),
                'vendor' => intval($matches[3], 16),
                'product' => intval($matches[4], 16),
                'name' => $matches[5]
            );
        }
    }
